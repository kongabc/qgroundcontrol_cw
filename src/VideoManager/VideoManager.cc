...
//    connect(ffmpegThread,&FFmpegThread::ffVideoStreamChanged, this, &VideoManager::onAllVideoStreamChanged);
    ...
    if(isExists){
        ...
        ffmpegThread->_allVideoStream = qSetFile->value("inpVal").toString();
        QString val = qSetFile->value("inpVal").toString();
...
//        qDebug()<<"set --changeValue2222------" << ffmpegThread->_allVideoStream ;
...
{
//    qDebug()<<"----_rtspUrlChanged()-----_restartVideo---";
//    qDebug()<<_videoUri[0];
    _restartVideo(0);
}
...
{
//    qDebug()<<"----VideoManager::_initVideo()--------";
#if defined(QGC_GST_STREAMING)
...
VideoManager::_startReceiver(unsigned id)
{
    ...
    // qDebug()<<"------VideoManager::_startReceiver()--id=" <<id << _videoUri[id];
